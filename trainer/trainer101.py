...
if torch.cuda.is_available():
    try:
        ...
        GPUS_LIST = ['cuda:0']#list(range(len(GPUS.split(','))))
    except:
        GPUS_LIST = None
else:
    GPUS_LIST = []

class Food101smallTrainer(object):
    def __init__(self):
        super(Food101smallTrainer, self).__init__()
        self.model = resnet18()
    ...
    def train(self, nb_epoch):
        ...
        for i, data in enumerate(trainstream):
            ...
            with torch.set_grad_enabled(True):
                ...
                acc = 100. * ((y.int().flatten() == out_labels.int().flatten()).sum().item() / y.size(0))
